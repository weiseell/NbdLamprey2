...
alllocs <- subset(alllocs_cohorts,alllocs_cohorts$loc != "STE")
alllocs <- alllocs %>% 
  select(-EstAge)
locs <- unique(alllocs$loc)
...
tiff(filename = "Figures/FamilyPlots_081523.tiff",height = 12,width = 15,units = "in",res = 400)
...
#alternate plot method with  single data frame instead of a loop
...
multiplot(cols = 3,Nsplots[[1]],Nsplots[[4]],Nsplots[[8]],
          Nsplots[[11]],Nsplots[[14]],Nsplots[[16]],
          Nsplots[[2]],Nsplots[[5]],Nsplots[[9]],
          Nsplots[[12]],Nsplots[[6]],Nsplots[[17]],
          Nsplots[[3]],Nsplots[[7]],Nsplots[[10]],
          Nsplots[[13]],Nsplots[[15]])
dev.off()

#Scatter plots with mixture analysis results
Nsplots[[6]]

##making per-stream plots from the already per-analysis plots from above ####
